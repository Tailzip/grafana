...
author:
  - Rémi REY (@rrey)
version_added: "2.10"
short_description: Manage Grafana Annotations
description:
  - Create/update/delete Grafana Annotations through API.
options:
  url:
    description:
      - The Grafana URL.
...
class GrafanaAnnotationService(object):
    ...
    def _send_request(self, url, data=None, headers=None, method="GET"):
        ...
        full_url = "{grafana_url}{path}".format(
            grafana_url=self.grafana_url, path=url)
        resp, info = fetch_url(self._module, full_url,
                               data=data, headers=headers, method=method)
        status_code = info["status"]
        if status_code == 404:
            return None
        elif status_code == 401:
            self._module.fail_json(failed=True, msg="Unauthorized to perform action '%s' on '%s' header: %s" % (
        ...
        self._module.fail_json(
            failed=True, msg="Grafana Annotations API answered with HTTP %d" % status_code)
...
argument_spec.update(
    ...
    url_password=dict(aliases=['grafana_password'],
